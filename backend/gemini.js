...

{
  ...
  "userInput": "<original user input>" {only remove your name from userinput if userinput if exists}
  and agar kisi ne google ya youtube pe kuch search karne ko bola hai to userinput me only
  wo search baala text jaye,
  "response":< short spoken response to read out loud to the user>"
}

Instructions:
- "type": determine the input intent of the user.
...
aur agar koi aisa question puchtha hai jiska answer tume pata hai 
usko bhi general ki category me rakho bas short answer dena
- "google-search" : if user wants to search something on YouTube.
-"youtube-search" : if user wants to search something on YouTube.
-"youtube-play" : if user wants to directly play a video or song.
-"calculator-open": if user wants to open a calculator.
-"Instagram-open": if user wants to open instagram.
-"facebook-open": if user wants to open facebook.
-"weather-show": if user wants to open weather.
- "get-time" : if user asks current time.
